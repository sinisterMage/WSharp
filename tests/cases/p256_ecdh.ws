...
fn main() i64 {
    ...
    print(bytes.to_hex(p256.derive(
    ...
    const gi = p256.derive(i) catch return 2;
    const gr = p256.derive(r) catch return 3;
    ...
    const secret = p256.ecdh(i, gr) catch return 4;
    print(bytes.to_hex(secret));
    if (bytes.equal(secret, p256.ecdh(r, gi) catch return 5)) {
    ...
    // to forget to test.
    if (p256.valid(gi) and p256.valid(gr)) { print("a derived key is a valid one"); }
    ...
    var n = 0;
    while (n < 4) : (n += 1) {
        const scratch = p256.ecdh(i, gr) catch return 6;
