...
for i in $(seq 1 "$ATTEMPTS"); do
    ...
    case "$MODE" in
        ...
        run)    "$TIMEOUT_BIN" "$TIMEOUT" "$WSHARP" run "$CASE" $args >"$OUT/o" 2>"$OUT/e" ;;
        # shellcheck disable=SC2086
        stress) "$TIMEOUT_BIN" "$TIMEOUT" "$WSHARP" run --gc-stress "$CASE" $args >"$OUT/o" 2>"$OUT/e" ;;
        # shellcheck disable=SC2086
        build)  "$TIMEOUT_BIN" "$TIMEOUT" "$exe" $args >"$OUT/o" 2>"$OUT/e" ;;
    esac
    code=$?
    e="$(date +%s)"
    printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$i" "$code" "$((e - s))" \
